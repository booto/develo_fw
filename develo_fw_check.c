...
void do_section(section_header *sh, FILE *f) {
  ...
  if (!si) {
    printf("unknown section; trying to skip\n");
    fseek(f, sh->length + 4, SEEK_CUR);
    return;
  }

  printf("section %s (length 0x%08x): ", si->desc, sh->length);
  crc = 0;
